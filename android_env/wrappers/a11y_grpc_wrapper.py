...
class A11yGrpcWrapper(base_wrapper.BaseWrapper):
    ...
    * If someone *has* called `task_extras()` since last step, the newly fetched
    ...
    def __init__(
        ...
        using_physical_device: bool = False
    ):
        """Initializes wrapper.
        ...
        """
        self._env = env
        self.using_physical_device = using_physical_device
        print("using_physical_device:", using_physical_device)
        ...
        self._servicer = a11y_servicer.A11yServicer(latest_forest_only=latest_a11y_info_only)
        a11y_pb2_grpc.add_A11yServiceServicer_to_server(self._servicer, self._server)
    ...
    def _start_a11y_services(self) -> None:
        ...
        if start_service_response.status != adb_pb2.AdbResponse.Status.OK:
            raise RuntimeError("Could not start accessibility forwarder " "service: " f"{start_service_response}.")
    ...
    def _install_a11y_forwarding_apk(self) -> None:
        ...
        if install_response.status != adb_pb2.AdbResponse.Status.OK:
            raise ValueError(f"Could not install accessibility_forwarder.apk: {install_response}.")
    ...
    def _enable_a11y_tree_logs(self) -> None:
        ...
        if enable_tree_logs_response.status != adb_pb2.AdbResponse.Status.OK:
            raise ValueError("Could not enable accessibility tree logging: " f"{enable_tree_logs_response}.")
        ...
        self.execute_adb_call(
            adb_pb2.AdbRequest(
                settings=adb_pb2.AdbRequest.SettingsRequest(
                    ...
                    put=adb_pb2.AdbRequest.SettingsRequest.Put(key="airplane_mode_on", value="0"),
                )
            )
        )
        time.sleep(1.0)
    ...
    def _configure_grpc(self) -> None:
        ...
        self.execute_adb_call(
            adb_pb2.AdbRequest(
                settings=adb_pb2.AdbRequest.SettingsRequest(
                    name_space=adb_pb2.AdbRequest.SettingsRequest.Namespace.GLOBAL,
                    put=adb_pb2.AdbRequest.SettingsRequest.Put(key="no_proxy", value=f'{"127.0.0.1" if self.using_physical_device else "10.0.2.2"}:{self._port}'),
                )
            )
        )
        time.sleep(3.0)
        ...
        self.execute_adb_call(
            adb_pb2.AdbRequest(
                send_broadcast=adb_pb2.AdbRequest.SendBroadcast(
                    action=(f'accessibility_forwarder.intent.action.SET_GRPC --ei "port" {self._port} --es "host" {"127.0.0.1" if self.using_physical_device else "10.0.2.2"}'),
                    component=(
                        "com.google.androidenv.accessibilityforwarder/com.google.androidenv.accessibilityforwarder.FlagsBroadcastReceiver"
                    ),
                )
    ...
    def _fetch_task_extras_and_update_observation(
        ...
        if timeout > 0.0:
            observation = self._accumulate_and_return_a11y_info(timeout, get_env_observation=True)
    ...
    def reset(self) -> dm_env.TimeStep:
        ...
        new_observation = self._fetch_task_extras_and_update_observation(timestep.observation, timeout)
    ...
    def step(self, action: Any) -> dm_env.TimeStep:
        ...
        new_observation = self._fetch_task_extras_and_update_observation(timestep.observation, timeout=timeout)
        timestep = timestep._replace(observation=new_observation)
        return timestep
    ...
    def _fetch_task_extras(self) -> dict[str, Any]:
        ...
        if (
            ...
            logging.warning("AccessibilityForwarder logged exceptions: %s", self._a11y_exception)

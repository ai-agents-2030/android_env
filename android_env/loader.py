...
from android_env.components.simulators.device import device_simulator
...
def _load_task(task_config: config_classes.TaskConfig) -> task_pb2.Task:
    ...
    match task_config:
        case config_classes.FilesystemTaskConfig():
            with open(task_config.path) as proto_file:
...
def load(config: config_classes.AndroidEnvConfig, device_serial: str) -> environment.AndroidEnv:
    ...
    match config.simulator:
        case config_classes.EmulatorConfig():
            ...
            if device_serial and not device_serial.startswith('emulator-'):
                print("Using Device Simulator")
                simulator = device_simulator.DeviceSimulator(config=config.simulator, device_serial=device_serial)
            else:
                print("Using Emulator Simulator")
...
def _process_emulator_launcher_config(
    ...
    launcher_config.android_avd_home = os.path.expanduser(launcher_config.android_avd_home)
    launcher_config.android_sdk_root = os.path.expanduser(launcher_config.android_sdk_root)
    launcher_config.emulator_path = os.path.expanduser(launcher_config.emulator_path)
    emulator_config.adb_controller.adb_path = os.path.expanduser(emulator_config.adb_controller.adb_path)
